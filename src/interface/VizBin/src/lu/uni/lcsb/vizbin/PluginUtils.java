...
public class PluginUtils {
    ...
    public final int PLUGINMODELVERSION = 1;
    public enum PLUGINSTATE {VALID, CANNOTLOAD, NOTVALID, NOTSUPPORTED};
    ...
    public PluginUtils(File _pluginPath) {
        pluginPath = _pluginPath;
    }
    
    public void setParentOptionsPanel(JPanel _panel){
        parentPluginOptionsPanel = _panel;
    }
    ...
    public ArrayList<String> listPlugins(){
        ...
        for (int i=0;i<filelist.length;i++) {
            if (filelist[i].getName().endsWith(".class")) pluginlist.add(filelist[i].getName().replace(".class", ""));
        }
        return pluginlist;
    }
    ...
    public PLUGINSTATE getPluginState(){
        ...
        if (plugin == null) return PLUGINSTATE.CANNOTLOAD;
        else {
            try {
                pluginModelVersion = plugin.getPluginModelVersion();
            }
            catch (Exception e) { e.printStackTrace(); return PLUGINSTATE.NOTVALID; };
            if ( pluginModelVersion == this.PLUGINMODELVERSION ) return PLUGINSTATE.VALID;
            else return PLUGINSTATE.NOTSUPPORTED;
        }
    }
    
    public ArrayList<ArrayList<Point2D>> getClusters(ArrayList<Point2D> pointList) {
        if (plugin == null) return null;
        else return ((ClusteringPlugin)plugin).getClusters(pointList);
    }
}