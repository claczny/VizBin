...
public class DataExporter {
    ...
    public static void exportCluster(JFrame parentFrame, String inFileName, List<Sequence> sequenceList) {
        File inFile = new File(inFileName);
        int pos;
        Thread progressThread;
        final JDialog dlg = new JDialog(parentFrame, "Sequence export progress", true);
        
        if (!sequenceList.isEmpty() ) { // only try to export if there's something to export
            File outFile = getSelectedFile();
            if (outFile != null) { // only try to export if the user chose an output file
                ...
                try {
                    ...
                    while (line != null) {
                        if (line.startsWith(">")) { // if we've encountered a new sequence start
                            seqCounter+=1;
