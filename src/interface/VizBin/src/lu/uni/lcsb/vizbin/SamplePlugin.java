 ...
 * 
 * Sample class to show how a Clustering plugin looks like.
 * The getClusters() method simply splits the given points in two clusters.
 * 
 */
public class SamplePlugin implements ClusteringPlugin {
    ...
    public ArrayList<ArrayList<Point2D>> getClusters(ArrayList<Point2D> pointList) {
        ...
        for (int i = 0; i<pointList.size()/2; i++) cluster.add(pointList.get(i));
        clusters.add(cluster);
        cluster = new ArrayList<Point2D>();
        for (int i = (pointList.size()/2)+1; i<pointList.size(); i++) cluster.add(pointList.get(i));
        clusters.add(cluster);
        return clusters;
    }
